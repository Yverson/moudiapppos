/**
 * Service de statistiques pour AppPOS (SQLite via Tauri uniquement).
 */
...
class StatsService {
  ...
  async getLocalStats(_period: 'day' | 'week' | 'month' = 'month'): Promise<StatsData> {
    ...
    const overview: StatsOverview = {
      ...
      revenueChange: 0, ordersChange: 0, avgOrderChange: 0,
    };

    const salesByPeriod = {
      today: { period: 'Today', revenue: todayRevenue, orders: todayOrders.length, avgOrder: todayOrders.length > 0 ? todayRevenue / todayOrders.length : 0, growth: '+0%' },
      week: { period: 'This Week', revenue: weekRevenue, orders: weekOrders.length, avgOrder: weekOrders.length > 0 ? weekRevenue / weekOrders.length : 0, growth: '+0%' },
      month: { period: 'This Month', revenue: monthRevenue, orders: monthOrders.length, avgOrder: monthOrders.length > 0 ? monthRevenue / monthOrders.length : 0, growth: '+0%' },
    };
    ...
    return { overview, salesByPeriod, topProducts, hourlySales, paymentMethods };
  }
  ...
  async getOnlineStats(restaurantId: string, period: 'day' | 'week' | 'month' = 'month'): Promise<StatsData | null> {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/restaurants/${restaurantId}/stats`, {
        params: { period },
  ...
  async getCombinedStats(period: 'day' | 'week' | 'month' = 'month'): Promise<{
    ...
    if (!online) return { local, online: null, combined: local };
    ...
    const combined: StatsData = {
      overview: {
        ...
        avgOrderValue: (local.overview.totalRevenue + online.overview.totalRevenue) / (local.overview.totalOrders + online.overview.totalOrders || 1),
  ...
  private async getLocalOrders(): Promise<Order[]> {
    try {
      ...
      return await tauriInvoke<Order[]>('get_orders', { restaurantId });
    } catch {
      return [];
    }
  }
  ...
  private async getLocalPayments(): Promise<Payment[]> {
    try {
      const restaurantId = this.restaurantId || '';
      return await tauriInvoke<Payment[]>('get_all_payments', { restaurantId });
    } catch {
      return [];
    }
  }
    ...
    orders.forEach(order => {
      try {
        ...
        items.forEach((item: any) => {
          ...
          productStats.set(item.name, { sold: existing.sold + (item.quantity || 0), revenue: existing.revenue + (item.price || 0) * (item.quantity || 0) });
        });
      } catch { /* ignore */ }
    });
    return Array.from(productStats.entries())
      .map(([name, stats]) => ({ name, sold: stats.sold, revenue: stats.revenue, trend: 'up' as const }))
      .sort((a, b) => b.revenue - a.revenue)
      .slice(0, 5);
  }

  private calculateHourlySales(orders: Order[]): HourlySales[] {
    const hourlySales = new Array(24).fill(0).map((_, hour) => ({ hour, revenue: 0, orders: 0 }));
    orders.forEach(order => {
  ...
  private calculatePaymentMethodStats(payments: Payment[]): PaymentMethodStats[] {
    ...
    payments.forEach(payment => {
      ...
      methodStats.set(method, { count: existing.count + 1, total: existing.total + payment.amount });
    });
    return Array.from(methodStats.entries())
      .map(([method, stats]) => ({ method, count: stats.count, total: stats.total, percentage: totalAmount > 0 ? (stats.total / totalAmount) * 100 : 0 }))
  ...
  private mergeSalesPeriod(local: SalesPeriod, online: SalesPeriod): SalesPeriod {
    ...
    return { period: local.period, revenue, orders, avgOrder: orders > 0 ? revenue / orders : 0, growth: online.growth };
  }
  ...
  private mergeTopProducts(local: TopProduct[], online: TopProduct[]): TopProduct[] {
    ...
    [...local, ...online].forEach(product => {
      ...
      if (existing) merged.set(product.name, { name: product.name, sold: existing.sold + product.sold, revenue: existing.revenue + product.revenue, trend: product.trend });
      else merged.set(product.name, product);
    });
    return Array.from(merged.values()).sort((a, b) => b.revenue - a.revenue).slice(0, 5);
  }
  ...
  private mergeHourlySales(local: HourlySales[], online: HourlySales[]): HourlySales[] {
    ...
    [...local, ...online].forEach(hourData => {
      ...
      if (existing) merged.set(hourData.hour, { hour: hourData.hour, revenue: existing.revenue + hourData.revenue, orders: existing.orders + hourData.orders });
      else merged.set(hourData.hour, hourData);
    });
  ...
  private mergePaymentMethods(local: PaymentMethodStats[], online: PaymentMethodStats[]): PaymentMethodStats[] {
    ...
    [...local, ...online].forEach(method => {
      ...
      if (existing) merged.set(method.method, { method: method.method, count: existing.count + method.count, total: existing.total + method.total, percentage: 0 });
      else merged.set(method.method, { ...method, percentage: 0 });
      totalAmount += method.total;
    });
    return Array.from(merged.values()).map(m => ({ ...m, percentage: totalAmount > 0 ? (m.total / totalAmount) * 100 : 0 })).sort((a, b) => b.total - a.total);
  }
}
